// shader制作粒子飞线 https://www.jianshu.com/p/7bdb71360899
...
export default class PointsFlyLine {
  ...
  geometry: THREE.BufferGeometry; // 曲线

  spline: THREE.CatmullRomCurve3; // 粒子系统

  particleSystem: THREE.Points; // 粒子数目

  pointNum: number; // 粒子间的总距离
...
  */
  ...
  constructor({
    ...
    this.distance = this.spline.getLength() // 初始化粒子
    ...
    for (let i = 0, z = 0; i < colorsLen; i += 4, z++) {
      ...
      colors[i + 3] = (i + 3) / sizeLen // size从小到大
  ...
  start() {
    ...
    this.tween = this.tweenAnimate(
      m,
      { start: max - end, end: max },
      2000,
      null,
      () => {
        let pointArr: number[] = []

        const s = Math.round(m.start)
        const e = Math.floor(m.end)
  ...
  tweenAnimate(
    ...
    var animate = animation || TWEEN.Easing.Linear.None
